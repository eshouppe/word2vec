...
def read_data(filename):
  ...
  with zipfile.ZipFile(filename) as f:
    # Namelist is the names of files in an existing archives
...
def build_dataset(words):
  count = [['UNK', -1]]
  # Add 50k most common words to list. most_common returns list of tuples
  count.extend(collections.Counter(words).most_common(vocabulary_size - 1))
  dictionary = dict()
  for word, _ in count:
    dictionary[word] = len(dictionary) # Key is vocab word & value is # of words in vocab
  data = list()
  unk_count = 0
  for word in words:
